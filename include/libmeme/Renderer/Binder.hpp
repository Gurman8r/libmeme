...
namespace ml
{
	...
	template <class T> struct Binder<T> final : public NonCopyable
	{
		...
		ML_NODISCARD inline operator bool() const noexcept { return m_value; }

		ML_NODISCARD inline operator pointer() noexcept { return const_cast<pointer>(m_value); }

		ML_NODISCARD inline operator const_pointer() const noexcept { return m_value; }

		ML_NODISCARD inline pointer operator->() noexcept { return const_cast<pointer>(m_value); }

		ML_NODISCARD inline const_pointer operator->() const noexcept { return m_value; }
