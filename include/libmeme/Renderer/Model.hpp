...
namespace ml
{
	...
	struct ML_RENDERER_API Model final : public Trackable
	{
		...
		ML_NODISCARD inline decltype(auto) operator[](size_type const i) { return m_storage[i]; }

		ML_NODISCARD inline decltype(auto) operator[](size_type const i) const { return m_storage[i]; }
